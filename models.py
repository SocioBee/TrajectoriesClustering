...
class AAMsoftmax(nn.Module):
    def __init__(self, n_class, m, s):
        ...
        self.m = m
        self.s = s
        self.weight = torch.nn.Parameter(torch.FloatTensor(n_class, 256), requires_grad=True).cuda()
...
class TrainableCentroidAAMSoftmaxLoss(nn.Module):
    def __init__(self, num_prototypes, embedding_dim, margin=0.35, scale=30):
        super(TrainableCentroidAAMSoftmaxLoss, self).__init__()
        self.margin = margin
        self.scale = scale
        self.prototypes = nn.Parameter(torch.FloatTensor(num_prototypes, embedding_dim))
    ...
    def forward(self, embeddings, counts_per_cluster):
        # Normalize embeddings
        embeddings_norm = F.normalize(embeddings, p=2, dim=1)
        prototypes_norm = F.normalize(self.prototypes, p=2, dim=1)

        # Compute cosine similarity between embeddings and prototypes
        cosine_sim = torch.matmul(embeddings_norm, prototypes_norm.T.cuda())

        # Apply angular margin
        sine_sim = torch.sqrt(1.0 - torch.pow(cosine_sim, 2)).clamp(0, 1)
        phi = cosine_sim * self.cos_m - sine_sim * self.sin_m
        phi = torch.where(cosine_sim - self.th > 0, phi, cosine_sim - self.mm)

        # Assign embeddings to closest prototypes for the purpose of loss calculation
        embeddings_to_prototype = cosine_sim.argmax(dim=1)
        one_hot_labels = F.one_hot(embeddings_to_prototype, num_classes=self.prototypes.size(0))

        output = one_hot_labels * phi + (1.0 - one_hot_labels) * cosine_sim
        output *= self.scale

        # Loss calculation assumes embeddings are assigned to the closest prototype
        loss = F.cross_entropy(output, embeddings_to_prototype)

        return loss

class DynamicClusterAAMSoftmaxLoss(nn.Module):
    def __init__(self, margin=0.75, scale=50):
        """
        Dynamic Cluster AAM-Softmax Loss for sorted embeddings with counts per cluster.

        Args:
        margin (float): Angular margin to enhance cluster separability.
        scale (float): Scaling factor for the logits.
        """
        super(DynamicClusterAAMSoftmaxLoss, self).__init__()
        self.margin = margin
        self.scale = scale
        self.cos_m = math.cos(margin)
        self.sin_m = math.sin(margin)
        # Threshold and adjustment as per AAM-Softmax
        self.th = math.cos(math.pi - margin)
        self.mm = math.sin(math.pi - margin) * margin

    def forward(self, embeddings, counts_per_cluster):
        """
        Forward pass for dynamically clustered embeddings.

        Args:
        embeddings (Tensor): Sorted embeddings with shape (batch_size, embed_dim).
        counts_per_cluster (List[int]): Number of examples per cluster.
        ...
        # Segment embeddings into clusters and compute centroids
        centroids = []
        start = 0
        for count in counts_per_cluster:
            cluster_embeddings = embeddings_norm[start:start + count]
            centroid = cluster_embeddings.mean(dim=0)
            centroids.append(centroid)
            start += count
        centroids = torch.stack(centroids)

        # Calculate cosine similarities to centroids
        centroids_norm = F.normalize(centroids, p=2, dim=1)
        cosine_sim = torch.matmul(embeddings_norm, centroids_norm.T)
        ...
        # Construct target similarity matrix
        target_sim = torch.zeros_like(cosine_sim)
        start = 0
        ...
        # Select the correct logits and apply margin
        output = target_sim * phi + (1.0 - target_sim) * cosine_sim
        output *= self.scale

        # Prepare labels for cross-entropy
        labels = torch.arange(len(counts_per_cluster)).repeat_interleave(counts_per_cluster.clone().detach().cpu())
        # Compute loss
...
class GE2ELoss(nn.Module):
    def __init__(self, init_w=10.0, init_b=-5.0, loss_method='contrast'):
    ...
    def forward(self, dvecs, counts_per_class):
        ...
        # Calculate loss
        loss = self.embed_loss_contrast(cos_sim_matrix, counts_per_class)
...
class DTC(nn.Module):
    def __init__(self, args, device, alpha=1):
        ...
        self.ge2elayer = GE2ELoss()
        self.amsftmax_layer = DynamicClusterAAMSoftmaxLoss(margin = 0.9, scale = 1)
